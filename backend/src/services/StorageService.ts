...
export class StorageService {
  createDisk(driveConfig: DriveConfig): Disk {
    switch (driveConfig.type) {
      case "local": {
        const cfg = driveConfig.config as LocalConfig;
        return new Disk(new FSDriver({ location: cfg.root, visibility: "public" }));
      }
      case "s3": {
      ...
      case "azure": {
        ...
        return new Disk(new AzureDriver({ connectionString: cfg.connectionString, container: cfg.container }));
      }
      default:
