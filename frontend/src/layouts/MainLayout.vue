...
import { Search01Icon, PowerIcon, Cancel01Icon, Menu01Icon, Sun02Icon, Moon02Icon } from '@hugeicons/core-free-icons'
  ...
  <div class="h-screen flex flex-col bg-page font-sans">
    ...
    <header class="flex items-center px-2 h-14 md:h-16 shrink-0">
      ...
      <div class="flex-1 max-w-180">
        <div class="flex items-center bg-panel rounded-2xl px-3 md:px-5 h-10 md:h-12 transition-all">
          <HugeiconsIcon :icon="Search01Icon" class="text-subtle mr-2 md:mr-3 shrink-0" :size="20" />
          <input
            type="text"
            :value="searchInput"
          ...
          <button
            v-if="searchInput"
            @click="clearSearch"
            class="text-muted hover:text-subtle ml-2"
          >
      ...
      <div class="flex items-center gap-1 md:gap-2 ml-auto shrink-0">
        <button
          ...
          title="Toggle theme"
        >
          <HugeiconsIcon :icon="themeStore.dark ? Sun02Icon : Moon02Icon" class="text-subtle" :size="20" />
        </button>
        <button
          @click="handleLogout"
          class="w-9 h-9 rounded-full flex items-center justify-center hover:bg-btn-hover transition-colors"
