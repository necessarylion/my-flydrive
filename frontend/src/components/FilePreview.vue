...
<template>
  ...
  >
    <div
      class="bg-panel rounded-2xl shadow-2xl w-[95vw] md:w-[90vw] max-w-5xl h-[95vh] md:h-[95vh] flex flex-col overflow-hidden"
    >
      <!-- Header -->
      <div
        class="flex items-center justify-between px-5 py-3 border-b border-divider-light shrink-0"
      >
        <h3 class="text-sm font-medium text-heading truncate" :title="fileName">
          {{ middleTruncate(fileName, 40) }}
        </h3>
      ...
      <div class="flex-1 overflow-auto flex items-center justify-center bg-panel-alt p-4">
        ...
        <div v-else-if="previewType === 'audio'" class="w-full max-w-lg">
          <div class="bg-panel rounded-xl p-8 shadow-sm text-center">
            ...
            >
              <svg
                class="w-10 h-10 text-purple-500 dark:text-purple-400"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
